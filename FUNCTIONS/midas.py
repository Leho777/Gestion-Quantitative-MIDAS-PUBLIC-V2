...
# =============================================================================
# SELF-CHECK TESTS (run with: python -m FUNCTIONS.midas)
# =============================================================================

def _self_check_weights():
    """
    Verify Almon weights implementation against paper Eq. 2.2.
    
    Paper: B(j; θ) = exp(θ·j) / Σexp(θ·i), j = 0, ..., m-1
    """
    import numpy as np
    
    # Test 1: Sum to 1
    for m in [21, 63, 126]:
        for theta in [-0.5, -0.1, 0.0, 0.1]:
            w = exp_almon_weights(m, theta)
            assert abs(w.sum() - 1.0) < 1e-10, f"Sum != 1 for m={m}, θ={theta}"
    
    # Test 2: All positive
    w = exp_almon_weights(63, -0.05)
    assert all(w > 0), "Weights not all positive"
    
    # Test 3: Monotonicity with θ < 0 (more weight on recent)
    w = exp_almon_weights(63, -0.1)
    assert w[-1] > w[0], "With θ<0, recent obs should have more weight"
    
    # Test 4: Monotonicity with θ > 0 (more weight on older)
    w = exp_almon_weights(63, 0.1)
    assert w[0] > w[-1], "With θ>0, older obs should have more weight"
    
    # Test 5: Uniform with θ = 0
    w = exp_almon_weights(63, 0.0)
    assert abs(w[0] - w[-1]) < 1e-10, "With θ=0, weights should be uniform"
    
    print("✓ exp_almon_weights: PASS")


def _self_check_midas_aggregate():
    """
    Verify midas_aggregate produces sensible values.
    """
    import numpy as np
    
    # Create dummy data: [0, 1, 2, ..., m-1]
    m = 63
    block = np.arange(m).astype(float)
    
    # With θ < 0, weighted average should be > midpoint (skewed to recent)
    theta = -0.1
    agg = midas_aggregate(block, theta)
    midpoint = (m - 1) / 2  # = 31
    assert agg > midpoint, f"Aggregation {agg} should be > midpoint {midpoint}"
    
    # With θ > 0, weighted average should be < midpoint (skewed to older)
    theta = 0.1
    agg = midas_aggregate(block, theta)
    assert agg < midpoint, f"Aggregation {agg} should be < midpoint {midpoint}"
    
    # With θ = 0, should equal arithmetic mean
    theta = 0.0
    agg = midas_aggregate(block, theta)
    assert abs(agg - midpoint) < 1e-10, f"With θ=0, agg should equal mean {midpoint}"
    
    print("✓ midas_aggregate: PASS")


def _self_check_index_convention():
    """
    Verify index convention matches paper:
    - k=0 → oldest observation (least weight if θ<0)
    - k=m-1 → most recent observation (most weight if θ<0)
    """
    import numpy as np
    ...
    # Verify monotonically increasing (since θ < 0)
    for i in range(len(w) - 1):
        assert w[i] < w[i+1], f"Weights not monotonically increasing at index {i}"
    
    # Block where each value equals its index
    block = np.arange(m).astype(float)
    
    # Weighted average should be closer to m-1 than to 0
    agg = np.dot(w, block)
    assert agg > (m - 1) / 2, f"Aggregation {agg} not skewed toward recent"
    
    print("✓ Index convention: PASS")


# =============================================================================
# FORMAL IMPULSE TESTS (Andreou et al. 2013, Eq. 2.2 validation)
# =============================================================================

def _test_impulse_recent():
    """
    TEST OBLIGATOIRE #1: Impulsion sur le jour le plus RÉCENT.
    
    Configuration:
    - block[0:m-1] = 0 (anciennes observations)
    - block[m-1] = 1 (dernière observation = jour le plus récent)
    
    Convention vérifiée:
    - Dans _extract_last_m(): block[-1] = dernier jour (le plus récent)
    - Avec θ < 0: le poids le plus fort doit être sur block[-1]
    
    Résultat attendu:
    - agg = w[-1] * 1 + Σ w[i] * 0 = w[-1]
    - w[-1] doit être le poids le plus élevé
    """
    import numpy as np
    
    m = 63
    theta = -0.1  # θ < 0 ⇒ plus de poids sur récent
    
    # Bloc avec impulsion sur le jour le plus récent
    block = np.zeros(m)
    block[-1] = 1.0  # Dernier jour = le plus récent dans notre convention
    ...
    # Vérifications
    expected_agg = w[-1]  # Seule observation non-nulle
    
    print(f"  Block: zeros sauf block[-1]=1 (jour récent)")
    print(f"  θ = {theta}")
    print(f"  w[0] (poids jour ancien)  = {w[0]:.6f}")
    print(f"  w[-1] (poids jour récent) = {w[-1]:.6f}")
    print(f"  Agrégation = {agg:.6f}")
    print(f"  Attendu    = {expected_agg:.6f}")
    
    assert abs(agg - expected_agg) < 1e-10, f"Agrégation {agg} != attendu {expected_agg}"
    assert w[-1] > w[0], f"Poids récent {w[-1]} devrait être > poids ancien {w[0]}"
    
    print("✓ Test impulsion RÉCENT: PASS")
    return agg


def _test_impulse_old():
    """
    TEST OBLIGATOIRE #2: Impulsion sur le jour le plus ANCIEN.
    
    Configuration:
    - block[0] = 1 (première observation = jour le plus ancien)
    - block[1:m] = 0 (observations récentes)
    
    Convention vérifiée:
    - Dans _extract_last_m(): block[0] = premier jour (le plus ancien)
    - Avec θ < 0: le poids le plus faible doit être sur block[0]
    
    Résultat attendu:
    - agg = w[0] * 1 + Σ w[i] * 0 = w[0]
    - w[0] doit être le poids le plus faible
    """
    import numpy as np
    ...
    # Bloc avec impulsion sur le jour le plus ancien
    block = np.zeros(m)
    block[0] = 1.0  # Premier jour = le plus ancien dans notre convention
    ...
    # Vérifications
    expected_agg = w[0]  # Seule observation non-nulle
    
    print(f"  Block: zeros sauf block[0]=1 (jour ancien)")
    print(f"  θ = {theta}")
    print(f"  w[0] (poids jour ancien)  = {w[0]:.6f}")
    print(f"  w[-1] (poids jour récent) = {w[-1]:.6f}")
    print(f"  Agrégation = {agg:.6f}")
    ...
    assert abs(agg - expected_agg) < 1e-10, f"Agrégation {agg} != attendu {expected_agg}"
    assert w[0] < w[-1], f"Poids ancien {w[0]} devrait être < poids récent {w[-1]}"
    
    print("✓ Test impulsion ANCIEN: PASS")
    return agg
...
def _test_impulse_comparison():
    """
    TEST DÉCISIF: Comparaison impulsion récent vs ancien.
    
    Si θ < 0, alors:
    - agg(impulsion récent) > agg(impulsion ancien)
    
    C'est le test DÉFINITIF de conformité avec le papier.
    """
    agg_recent = _test_impulse_recent()
    print()
    agg_old = _test_impulse_old()
    
    print()
    print("-" * 50)
    print(f"  Agrégation (impulsion jour récent): {agg_recent:.6f}")
    print(f"  Agrégation (impulsion jour ancien): {agg_old:.6f}")
    print(f"  Ratio recent/ancien: {agg_recent/agg_old:.2f}x")
    
    assert agg_recent > agg_old, (
        f"ERREUR CRITIQUE: avec θ<0, l'impulsion sur le jour récent ({agg_recent:.6f}) "
        f"devrait donner une agrégation SUPÉRIEURE à l'impulsion sur le jour ancien ({agg_old:.6f})"
    )
    
    print()
    print("✓ Test comparaison impulsions: PASS")
    print("  → Le jour RÉCENT reçoit bien PLUS de poids que le jour ANCIEN avec θ<0")


if __name__ == "__main__":
    print("=" * 60)
    print("MIDAS Self-Checks - Conformity with Andreou et al. (2013)")
    print("=" * 60)
    
    print("\n--- Tests basiques ---")
    _self_check_weights()
    _self_check_midas_aggregate()
    _self_check_index_convention()
    
    print("\n--- Tests d'impulsion (preuve formelle) ---")
    _test_impulse_comparison()
